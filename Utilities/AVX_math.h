...
public:
    ...
    Vector3f8(const std::array<float, 3> &x)
    {
        v[0].v = _mm256_set1_ps(x[0]);
        v[1].v = _mm256_set1_ps(x[1]);
        v[2].v = _mm256_set1_ps(x[2]);
    ...
    Vector3f8(const glm::vec4 &x)
    {
        v[0].v = _mm256_set1_ps(x.x);
        v[1].v = _mm256_set1_ps(x.y);
        v[2].v = _mm256_set1_ps(x.z);
    }

    Vector3f8(const Vector &v0, const Vector &v1, const Vector &v2, const Vector &v3, const Vector &v4, const Vector &v5, const Vector &v6, const Vector &v7)
	{
    ...
    inline void store(std::vector<glm::vec3>& Vf) const
	{
		for (int i = 0; i < 3; i++)
		{
			float val[8];
    ...
    inline void store(glm::vec3* Vf) const
	{
		for (int i = 0; i < 3; i++)
		{
			float val[8];
...
static inline Vector3f8 convertVec_zero(const unsigned int *idx, const glm::vec3 *v, const unsigned char count = 8u)
{
	Vector3f8 x;
	switch (count)
	{
...
static inline Vector3f8 convertVec_zero(const unsigned int *idx, const std::array<float, 3> *v, const unsigned char count = 8u)
{
    Vector3f8 x;
    switch (count)
    {
...
static inline Vector3f8 convertVec_zero(const unsigned int *idx, const glm::vec4 *v, const unsigned char count = 8u)
{
    Vector3f8 x;
    switch (count)
    {
