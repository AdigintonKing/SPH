...
const float ARROW_SHAFT_THICKNESS = 4.0;
...
float time;
vec2 v (vec4 b) {
        return 20.*(vec2(b.x-b.y,b.z-b.w));
}
...
float ArrowFactor = (1.0-arrow2(gl_FragCoord.xy, f_vel.xy * ARROW_TILE_SIZE *0.25));
if(ArrowFactor>0.15)
   FragColor = vec4(1.0);
else
    FragColor  =  vec4(color_,fColor[3]);
}
