...
public:
  ...
  std::vector<float> p;
    // Temperature
  std::vector<float> T;
  // temperature gradient and condutivity constant
  std::vector<float> dt;
    ...
    //}else tdif = Tmax - Tmin;
     ...
     /*if(hueconta == 61){
         ...
         << " Hueconta: " << hueconta
                   ...
                   << " m: " << m
                   << " ratio: " << ratio
                   << " temp inicial : " << temperaturapx
                   << " mod = "<< fmod(hueconta / 60.0, 2)
                   << std::endl;
        }*/
...
void initFromCloud(float jitter = 0.02, float spacing = 0.015){
    ...
    for(int l = 0; l < pos.size(); l++){ // Teste de cores sem alteracao
        ...
        //Teste Celsius
        glm::vec4 testeCelsius;
        testeCelsius = CelsiustoRGB(particles.T[l], l, 1);

        particles.C[l][0]= testeCelsius[0];
            ...
            for (int i = 0; i < n_particles(); ++i) {
                ...
                //{
                    //std::cout  << "Ponto "<< i
                              ...
                              //<< ", temperatura nova: " <<  particles.T[i]
                              //<<", hue: " << particles.colorh[i]
                              //<< " Delta: " << particles.dt[i]
                             // << " R: " << particles.C[i][0]
                             // << " G: " << particles.C[i][1]
                             // << " B: " << particles.C[i][2]
                              //<< std::endl;
                ...
                glm::vec4 Colortmp = CelsiustoRGB(particles.T[i], i, 0);
                float newHue = CelsiustoHue(particles.T[i]);

                glm::vec4 OLDColortmp;
                float oldHue = particles.colorh[i];

                OLDColortmp[0] = particles.C[i][0];
                OLDColortmp[1] = particles.C[i][1];
                OLDColortmp[2] = particles.C[i][2];
