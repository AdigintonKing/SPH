...
public:
    ...
    inline void store(std::vector<Vector>& Vf) const
	{
		for (int i = 0; i < 3; i++)
		{
			float val[8];
    ...
    inline void store(Vector* Vf) const
	{
		for (int i = 0; i < 3; i++)
		{
			float val[8];
...
static inline Vector3f8 convertVec_zero(const unsigned int *idx, const Vector *v, const unsigned char count = 8u)
{
	Vector3f8 x;
	switch (count)
	{
